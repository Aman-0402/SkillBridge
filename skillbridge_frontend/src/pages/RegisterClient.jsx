...
const IDENTITIES = ['Enterprise', 'MSME', 'Professional', 'Self-Employed', 'Individual']
...
const STEPS = [
  { label: 'Personal Info', fields: ['fullName', 'phone', 'username', 'email'] },
  { label: 'Business Profile', fields: ['identity', 'state', 'interest1', 'interest2', 'interest3'] },
  { label: 'Security', fields: ['password', 'password2'] },
]
...
function validate(step, form) {
  ...
  if (step === 1) {
    ...
    if (!form.interest1) errs.interest1 = 'Please select Area of Interest 1.'
    if (!form.interest2) errs.interest2 = 'Please select Area of Interest 2.'
    else if (form.interest2 === form.interest1) errs.interest2 = 'Choose a different area from Interest 1.'
    if (!form.interest3) errs.interest3 = 'Please select Area of Interest 3.'
    else if (form.interest3 === form.interest1 || form.interest3 === form.interest2) errs.interest3 = 'Choose a different area from Interest 1 and 2.'
  }
  ...
  return (
    <section className="relative -mt-24 min-h-screen overflow-hidden bg-slate-950 px-4 pb-16 pt-32 lg:px-8">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1fr_1.2fr] lg:items-start">
        ...
        >
          <p className="text-xs font-black uppercase tracking-[0.24em] text-blue-400">Solution Seeker</p>
          <h1 className="mt-4 text-4xl font-black leading-tight tracking-tight text-white">
            Find the right expert for every challenge.
          </h1>
          <p className="mt-5 text-lg leading-8 text-slate-400">
            Post projects, book consulting sessions, and manage your business growth — all in one place.
          </p>
          <div className="mt-10 space-y-4">
            {[
              'Post projects & invite top freelancers',
              'Book expert consulting sessions',
              'Track milestones & manage payments',
            ...
            <div className="mt-6 grid gap-4">
              <div className="grid items-start gap-4 sm:grid-cols-2">
                ...
                <label className="grid gap-1.5 text-xs font-black uppercase tracking-[0.14em] text-slate-400">
                  Phone Number *
                  <span className="relative">
                    <FaPhone className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-blue-500" />
              ...
              <button type="button" onClick={handleNext}
                className="mt-2 inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-black text-white shadow-[0_14px_36px_rgba(37,99,235,0.3)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-blue-500">
            ...
            <div className="mt-6 grid gap-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="grid gap-1.5 text-xs font-black uppercase tracking-[0.14em] text-slate-400">
                  Identity *
                  <span className="relative">
                    <FaBuilding className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-blue-500" />
                ...
                <label className="grid gap-1.5 text-xs font-black uppercase tracking-[0.14em] text-slate-400">
                  State *
                  <span className="relative">
                    <FaLocationDot className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-blue-500" />
              ...
              <label className="grid gap-1.5 text-xs font-black uppercase tracking-[0.14em] text-slate-400">
                Area of Interest 1 *
                <span className="relative">
                  <FaStar className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-blue-500" />
                  <select name="interest1" value={form.interest1} onChange={handleChange} className={selectClass('interest1')}>
                    <option value="">Select Industry</option>
                    {INDUSTRIES.map(ind => <option key={ind} value={ind}>{ind}</option>)}
                  </select>
                </span>
                {errors.interest1 && <span className="text-xs font-bold text-rose-500">{errors.interest1}</span>}
              </label>
              ...
              <label className="grid gap-1.5 text-xs font-black uppercase tracking-[0.14em] text-slate-400">
                ...
                <span className="relative">
                  <FaStar className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-blue-500" />
                  <select name="interest2" value={form.interest2} onChange={handleChange} className={selectClass('interest2')}>
                    <option value="">Select Industry</option>
                    {INDUSTRIES.filter(ind => ind !== form.interest1).map(ind => <option key={ind} value={ind}>{ind}</option>)}
                  </select>
                </span>
                {errors.interest2 && <span className="text-xs font-bold text-rose-500">{errors.interest2}</span>}
              </label>
              ...
              <label className="grid gap-1.5 text-xs font-black uppercase tracking-[0.14em] text-slate-400">
                ...
                <span className="relative">
                  <FaStar className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-blue-500" />
                  <select name="interest3" value={form.interest3} onChange={handleChange} className={selectClass('interest3')}>
                    <option value="">Select Industry</option>
                    {INDUSTRIES.filter(ind => ind !== form.interest1 && ind !== form.interest2).map(ind => <option key={ind} value={ind}>{ind}</option>)}
                  </select>
                </span>
                {errors.interest3 && <span className="text-xs font-bold text-rose-500">{errors.interest3}</span>}
              </label>
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} className="mt-6 grid gap-4">
              ...
              <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4 text-xs text-slate-400">
                <p className="mb-2 font-black uppercase tracking-wider text-slate-300">Account Summary</p>
