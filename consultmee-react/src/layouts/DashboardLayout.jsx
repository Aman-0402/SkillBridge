...
const sidebarLinks = [
  ...
  { label: 'Consultants', to: '/dashboard?section=consultants', icon: FaUserTie },
  { label: 'Appointments', to: '/dashboard?section=appointments', icon: FaCalendarCheck },
  { label: 'Projects', to: '/dashboard?section=projects', icon: FaFolderOpen },
  { label: 'History', to: '/dashboard?section=history', icon: FaClockRotateLeft },
  { label: 'Profile', to: '/dashboard?section=profile', icon: FaUser },
]
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950 lg:grid lg:grid-cols-[280px_1fr]">
      ...
      <aside className="sticky top-0 z-30 hidden h-screen overflow-y-auto border-r border-white/10 bg-slate-950 px-4 py-5 text-white lg:block">
        ...
        <button className="mt-8 flex w-full items-center gap-3 rounded-lg px-4 py-3 text-left text-sm font-semibold text-rose-200 hover:bg-rose-500/10">
          <FaRightFromBracket />
          Logout
        </button>
      </aside>
